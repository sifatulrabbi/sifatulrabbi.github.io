  ...
  return (
    <div className="flex justify-end">
      <div className="max-w-full lg:max-w-[75%]">
        <div className="bg-terminal-accent text-terminal-background px-4 py-3 rounded-2xl rounded-tr-sm">
          <p className="whitespace-pre-wrap">{step.content.text}</p>
          {step.attachments && step.attachments.length > 0 && (
            <div className="mt-2 pt-2 border-t border-terminal-background/20">
              <p className="text-xs opacity-80 mb-1">Attachments:</p>
              <div className="flex flex-wrap gap-1">
                {step.attachments.map((att) => (
                  <span
                    key={att.id}
                    className="text-xs px-2 py-1 bg-terminal-background/20 rounded"
                  >
                    {att.name}
                  </span>
                ))}
